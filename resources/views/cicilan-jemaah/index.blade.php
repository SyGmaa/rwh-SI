    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h4>Cicilan Jemaah @if($jenisId) - {{ $jenisPakets->where('id', $jenisId)->first()->nama_jenis ?? '' }} @endif
          </h4>
          ...
          <div class="mb-3">
            ...
            <select id="jadwal_filter" class="form-control" onchange="filterByJadwal(this.value)">
              ...
              <option value="{{ $jadwal->id }}" {{ $jadwalId==$jadwal->id ? 'selected' : '' }}>{{
                $jadwal->paket->nama_paket }} - {{ $jadwal->tgl_berangkat->format('d/m/Y') }}</option>
              @endforeach
            </select>
          </div>
                ...
                <tr>
                  ...
                  <td>
                    ...
                    <button type="button" class="btn btn-sm btn-info"
                      onclick="viewBuktiBayar('{{ asset('storage/' . $cicilan->bukti_bayar) }}')">Lihat Bukti</button>
                    @else
                    -
                    @endif
                  </td>
                  <td>
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu">
                        <a href="{{ route('cicilan-jemaah.edit', $cicilan->id) }}" class="dropdown-item has-icon"><i
                            class="far fa-edit"></i> Edit</a>
                        <a href="{{ route('cicilan-jemaah.cetak', $cicilan->id) }}" target="_blank"
                          class="dropdown-item has-icon"><i class="fas fa-print"></i> Cetak Kwitansi</a>
                        {{-- <div class="dropdown-divider"></div> --}}
                        <form id="delete-form-{{ $cicilan->id }}"
                          action="{{ route('cicilan-jemaah.destroy', $cicilan->id) }}" method="POST"
                          style="display: inline;">
                          @csrf
                          @method('DELETE')
                          {{-- <a href="#"><button type="submit" class="dropdown-item has-icon text-danger"
                              onclick="return confirm('Apakah Anda yakin ingin menghapus cicilan jemaah ini?')"
                              style="border: none; background: none;"><i class="far fa-trash-alt"></i>
                              Delete</button></a> --}}
                          <a href="#" class="dropdown-item has-icon text-danger"
                            onclick="event.preventDefault(); if(confirm('Apakah Anda yakin ingin menghapus Cicilan jemaah ini?')) { document.getElementById('delete-form-{{ $cicilan->id }}').submit(); }"><i
