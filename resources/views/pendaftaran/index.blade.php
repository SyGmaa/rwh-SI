    ...
    <div class="col-12">
      <div class="card">
        <div class="card-header">
          <h4>Pendaftaran @if($jenisId) - {{ $jenisPakets->where('id', $jenisId)->first()->nama_jenis ?? '' }} @endif
          </h4>
          <div class="card-header-action">
            <a href="{{ route('pendaftaran.create') }}" class="btn btn-primary">Tambah Pendaftaran</a>
            @foreach($jenisPakets as $jenis)
            <a href="{{ route('pendaftaran.create', ['jenis_id' => $jenis->id]) }}" class="btn btn-outline-primary">{{
              $jenis->nama_jenis }}</a>
            @endforeach
          </div>
        </div>
            ...
            <table class="table table-striped" id="table-1">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ $pendaftaran->jadwalKeberangkatan->paket->nama_paket }} - {{
                    $pendaftaran->jadwalKeberangkatan->tgl_berangkat->format('d/m/Y') }}</td>
                  ...
                  <td>
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu">
                        <a href="{{ route('pendaftaran.show', $pendaftaran->id) }}" class="dropdown-item has-icon"><i
                            class="fas fa-eye"></i> View</a>
                        <a href="{{ route('pendaftaran.edit', $pendaftaran->id) }}" class="dropdown-item has-icon"><i
                        ...
                        <form id="delete-form-{{ $pendaftaran->id }}"
                          ...
                          {{-- <a href="#"><button type="submit" class="dropdown-item has-icon text-danger"
                              onclick="return confirm('Apakah Anda yakin ingin menghapus Pendaftaran jemaah ini?')"
                              style="border: none; background: none;"><i class="far fa-trash-alt"></i>
                              Delete</button></a> --}}
                          <a href="#" class="dropdown-item has-icon text-danger"
                            onclick="event.preventDefault(); if(confirm('Apakah Anda yakin ingin menghapus Pendaftaran jemaah ini?')) { document.getElementById('delete-form-{{ $pendaftaran->id }}').submit(); }"><i
