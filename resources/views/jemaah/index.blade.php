          ...
          <div class="mb-3">
            ...
            <select id="jadwal_filter" class="form-control" onchange="filterByJadwal(this.value)">
              ...
              <option value="{{ $jadwal->id }}" {{ $jadwalId==$jadwal->id ? 'selected' : '' }}>{{
                $jadwal->paket->nama_paket }} - {{ $jadwal->paket->jml_hari }} Hari - Rp {{
                number_format($jadwal->paket->harga, 0, ',', '.') }} - {{ $jadwal->tgl_berangkat->format('d/m/Y') }}
              </option>
              @endforeach
            </select>
          </div>
            ...
            <table class="table table-striped" id="table-1">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>{{ $jemaah->no_tlp ?? '-' }}</td>
                  <td>{{ ($jemaah->jadwal_override_id ? $jemaah->jadwalOverride->tgl_berangkat :
                  ...
                  <td>
                    <div class="dropdown">
                      ...
                      <div class="dropdown-menu">
                        ...
                        <form id="delete-form-{{ $jemaah->id }}" action="{{ route('jemaah.destroy', $jemaah->id) }}"
                          ...
                          <a href="#" class="dropdown-item has-icon text-danger"
                            onclick="event.preventDefault(); if(confirm('Apakah Anda yakin ingin menghapus Pendaftaran jemaah ini?')) { document.getElementById('delete-form-{{ $jemaah->id }}').submit(); }"><i
